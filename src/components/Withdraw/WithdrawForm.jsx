...
//         {/* Account Number */}
//         <div>
//           <label className="block text-sm font-medium text-Main">Seller ID</label>
//           <input
//             type="text"
//             value={accountNumber}
...
const WithdrawForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch("https://y-balash.vercel.app/api/seller/withdrawal/request", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="container p-4">
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-6">
        ...
        >
          <span>{loading ? "Processing..." : "WITHDRAW Request"}</span>
          <FaLongArrowAltRight className="size-6" />
        </button>
      </form>
    </div>
