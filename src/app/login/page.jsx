...
const LoginPage = () => {
  ...
  const { setToken } = useContext(authContext); // استخدم AuthContext
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("https://y-balash.vercel.app/api/auth/login", {
        email,
        password,
      });
      ...
      if (token) {
        setToken(token); // خزنه في السياق
        localStorage.setItem("token", token); // لو حابب تخزنه في localStorage كمان
  ...
  return (
    <div className="flex items-center justify-center p-6 mt-6">
      <div className="bg-white px-8 rounded-lg shadow-md max-w-md w-full text-center">
        <div className="flex justify-center">
          <Image src="/logo2.png" alt="yKaash Logo" width={150} height={20} priority style={{ width: "140px" }} />
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="relative">
            <label htmlFor="password" className="block text-left text-sm font-medium text-[#374151] ml-1 mb-1">
              Password
            </label>
            <input
              type={showPassword ? "text" : "password"}
